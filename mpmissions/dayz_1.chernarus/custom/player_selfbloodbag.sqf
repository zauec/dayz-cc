...
if(_bloodbagTime < _bloodbagLastUsedTime) exitWith { // If cooldown is not done then exit script
	cutText [format["Вы не можете сейчас залить себе кровь, попробуйте сделать это позже через %1!",(_bloodbagTime - _bloodbagLastUsedTime)], "PLAIN DOWN"]; //display text at bottom center of screen when players cooldown is not done
};

if (dayz_combat == 1) then { // Check if in combat
    cutText [format["Вы не можете сейчас залить себе кровь, вы находитесь в режиме боя!"], "PLAIN DOWN"]; //display text at bottom center of screen when in combat
} else {
	...
	if (_finished) then {
		...
		if (random(_infectionChance) < 1) then {
			...
			cutText [format["Вы залил кровь себе, но она была заражена!"], "PLAIN DOWN"]; //display text at bottom center of screen if infected
			...
			cutText [format["Вы залили себе кровь!"], "PLAIN DOWN"]; //display text at bottom center of screen on succesful self bloodbag
		};
		...
		cutText [format["Вы прервали переливание крови!"], "PLAIN DOWN"]; //display text at bottom center of screen on interrupt
	};
};